...
class CarTypesList extends React.Component {
            ...
            <TouchableOpacity style={styles.touchableOpacitySelection} onPress={() => this.selectedJobCatgoryId(item)}>
                {
                    this.state.lang_type == 'BD' ?
                        <Text style={styles.touchableText}>{item.car_type_name_bn}</Text>
                        :
                        <Text style={styles.touchableText}>{item.car_type_name_en}</Text>
                }
            </TouchableOpacity>
        );
    ...
    selectedJobCatgoryId(item) {
        this.setState({ selectedCarTypeId: item.car_type_id });
        this.setState({ selectedText: this.state.lang_type == 'BD' ? item.car_type_name_bn : item.car_type_name_en })
        this.setState({ isVisible: false });
        this.props.updateCarTypeState(item.car_type_id);
    }
    // hide show modal
    displayModal(show) {
                ...
                >
                    <View style={styles.modalView}>
                        <TouchableOpacity
                            ...
                            />
                            <FlatList
                                data={this.state.allCarTypes}
                                renderItem={(item) => this.renderItem(item.item)}
                                keyExtractor={item => item.car_type_id.toString()}
                                numColumns={1}
                            />
                        </TouchableOpacity>
                    </View>
